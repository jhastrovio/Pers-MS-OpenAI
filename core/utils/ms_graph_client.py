...
class GraphClient:
    ...
    async def list_files_in_folder(self, folder_path: str) -> list[dict]:
        """List files in a specific OneDrive folder."""
        logger.info(f"Listing files in OneDrive folder: {folder_path}")
        access_token = await self._get_access_token()
        headers = {"Authorization": f"Bearer {access_token}"}
        user_email = get_env_variable('user_email')
    ...
    async def download_file_from_onedrive(self, folder_path: str, file_name: str) -> bytes:
        """Download a file from OneDrive by folder_path and file_name."""
        logger.info(f"Downloading file {file_name} from folder {folder_path}")
        access_token = await self._get_access_token()
        headers = {"Authorization": f"Bearer {access_token}"}
        user_email = get_env_variable('user_email')
        # Remove leading/trailing slashes for safety
        folder_path = folder_path.strip("/")
        url = f"https://graph.microsoft.com/v1.0/users/{user_email}/drive/root:/{folder_path}/{file_name}:/content"
